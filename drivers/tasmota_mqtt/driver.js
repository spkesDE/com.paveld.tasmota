...
class TasmotaDeviceDriver extends GeneralTasmotaDriver {
  ...
  registerRunListeners() {
    ...
    multiSocketTrigger.registerRunListener((args, state) => {
      return Promise.resolve(
        (args.socket_id.name === "any socket" || args.socket_id.name === state.socket_id.name) &&
    ...
    this.homey.flow.getConditionCard("multiplesockets_all_switches_turned_on").registerRunListener((args, state) => {
      for (let socketIndex = 1; socketIndex <= args.device.relaysCount; socketIndex++) {
        if (!args.device.getCapabilityValue("switch." + socketIndex.toString())) return Promise.resolve(false);
      }
      return Promise.resolve(true);
    });
    this.homey.flow.getConditionCard("multiplesockets_some_switches_turned_on").registerRunListener((args, state) => {
      for (let socketIndex = 1; socketIndex <= args.device.relaysCount; socketIndex++) {
        if (args.device.getCapabilityValue("switch." + socketIndex.toString())) return Promise.resolve(true);
      }
  ...
  onMapDeviceClass(device) {
    ...
    if (settings.swap_prefix_topic) topic = topic + "/cmnd/" + command;
    else topic = "cmnd/" + topic + "/" + command;
  ...
  getDefaultIcon(settings, capabilities) {
    if (capabilities.includes("zigbee_pair")) return "zigbee_bridge.svg";
    if (capabilities.includes("multiplesockets")) return "extension_plug.svg";
    if (capabilities.includes("windowcoverings_state")) return "curtains.svg";
    if (capabilities.includes("singlesocket")) {
      if (capabilities.includes("fan_speed")) return "table_fan.svg";
      if (capabilities.includes("dim")) return "lightbulb.svg";
      return "electrical_outlet_1.svg";
    }
    if (capabilities.includes("additional_sensors")) return "sensor.svg";
    return "tasmota.svg";
  }
  ...
  collectedDataToDevice(deviceTopic, messages, swapPrefixTopic) {
    ...
    if ("Status" in messages) {
      if (messages["Status"][0]["DeviceName"] !== undefined) devItem.name = messages["Status"][0]["DeviceName"];
      else if (messages["Status"][0]["FriendlyName"] !== undefined) devItem.name = messages["Status"][0]["FriendlyName"][0];
      else devItem.name = deviceTopic;
    ...
    if ("StatusSNS" in messages) {
      ...
      Sensor.forEachSensorValue(
        messages["StatusSNS"][0],
        (path, value) => {
          let sensorField = path[path.length - 1];
          let sensor = "";
          if (path.length > 1) sensor = path[path.length - 2];
          ...
          if (capObj !== null) {
            ...
            if (capObj.units) {
              ...
              if (units_field !== null && units_field in messages["StatusSNS"][0]) units = messages["StatusSNS"][0][units_field];
              units = capObj.units.units_template.replace("{value}", units);
              capSettings["units"] = { en: units };
            }
            if (capObj.caption) capSettings["title"] = { en: capObj.caption };
            if (Object.keys(capSettings).length > 0) devItem.capabilitiesOptions[capObj.capability] = capSettings;
            if (sensorField in sensors_settings) sensors_settings[sensorField] = sensors_settings[sensorField] + 1;
            else sensors_settings[sensorField] = 1;
      ...
      if (Object.keys(sensors_settings).length > 0) {
        ...
        for (const sitem in sensors_settings)
          if (sensors_settings[sitem] > 1) sens_string.push(sitem + " (x" + sensors_settings[sitem] + ")");
          else sens_string.push(sitem);
    ...
    if ("StatusSTS" in messages) {
      ...
      if (relaysCount === 1) {
        ...
        if (lmCounter === 2) capabilities.push("light_mode");
        if ("FanSpeed" in messages["StatusSTS"][0]) {
          devItem.class = "fan";
          devItem.capabilities.push("fan_speed");
          devItem.settings.has_fan = "Yes";
        }
      } else if (relaysCount > 1) devItem.class = "other";
    }
    if (isZigbeeBridge) devItem.class = "other";
    ...
    if (devItem.capabilities.length <= 1) return null;
